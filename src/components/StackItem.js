...
export default function StackItem({data}) {
    ...
    return (
        <Box className="child" sx={{display: "flex", flexDirection: "column", alignItems: "center", justifyContent:"center"}}>
            <Avatar alt={data.name} src={data.logo} sx={{height: "115px", width: "110px" ,borderRadius: 0}}/>
            <Typography variant="h6" sx={{color: "white", fontSize:"20px"}} gutterBottom>{data.name}</Typography>
        </Box>
